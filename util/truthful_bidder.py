...
class Truthful_bidder():
    def __init__(self, loss='log', penalty='l2', alpha=1e-6, verbose=0, n_jobs=4, max_iter=100):
    ...
    def fit(self, x, y, z):
        ...
        return self.model, self.eCPC

    def bid(self, x):
        return np.ceil(self.model.predict_proba(x)[:, 1] * self.alpha * self.eCPC)
